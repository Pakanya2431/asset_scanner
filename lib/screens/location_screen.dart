...
class _LocationScreenState extends State<LocationScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabs,
        // Prevent swiping while camera is active to avoid lifecycle issues
...
// ── Tab 0: Location picker ────────────────────────────────────────────────────
class _LocationPickerTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rooms =
        building.isNotEmpty ? state.roomsForBuilding(building) : <String>[];
      ...
      children: [
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _sectionLabel(Icons.business_rounded, 'Select Building'),
                const SizedBox(height: 10),
                if (!state.hasDatabaseLoaded)
                  _hint('Import an Excel database first'),
                ...buildings.map((b) {
                  final bItems = state.allItems.where((s) => s.item.building == b);
                  ...
                  return _BuildingCard(
                    ...
                    onTap: () {
                      ...
                      onBuildingChanged(b, rooms.isNotEmpty ? rooms.first : '');
                    },
                  );
                }),
                const SizedBox(height: 16),
                _sectionLabel(Icons.door_front_door_rounded, 'Select Room'),
                const SizedBox(height: 10),
                if (rooms.isEmpty) _hint('Select a building first'),
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: rooms.map((r) {
                    final rItems = state.allItems.where(
                        (s) => s.item.building == building && s.item.room == r);
        ...
        Padding(
          ...
          child: ElevatedButton.icon(
            ...
            onPressed:
                (building.isEmpty || room.isEmpty) ? null : onSave,
          ),
        ),
      ],
    );
...
class _ScanTab extends StatelessWidget {
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Row(children: [
            _chip('Missing', missingCount, kRed, const Color(0xFFFEF2F2)),
            const SizedBox(width: 6),
            _chip('Found', foundCount, kGreen, const Color(0xFFF0FDF4)),
            const SizedBox(width: 6),
  ...
  Widget _chip(String label, int count, Color color, Color bg) => Expanded(
        child: Container(
...
// ── Compact item row for Location Mode ───────────────────────────────────────
          ...
          children: [
            ...
            if (item.isScanned && item.scannedAt != null) ...[
              ...
              Text(timeFmt.format(item.scannedAt!),
                  style:
                      const TextStyle(fontSize: 10, color: kTextHint)),
            ],
          ],
        ),
      ]),
